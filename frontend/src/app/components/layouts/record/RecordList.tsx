...
export default function RecordList() {
    ...
    const [editMenu_open, setEditMenu_open] = useState(false);
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            if (event.key === 'Escape' && (addMenu_open || editMenu_open)) {
                setAddMenu_open(false);
                setEditMenu_open(false);
    ...
    const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/record/${editingRecord.record_id}/`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
    ...
    return (
        <div className="w-2/3 justify-center items-center border-2 border-gray-300 rounded-md p-4 bg-white">
            ...
            {editMenu_open && editingRecord && (
                <div className="fixed bg-black/20 w-full h-full top-0 left-0 z-10">
                    <div className="fixed bg-white border-1 rounded-md border-[#E2E8F0] shadow-md w-1/3 h-1/3 top-1/3 left-1/3">
                        <button className="relative bg-[#DEDEDE] text-black font-normal rounded-md border-1 border-[#C2C2C2] px-2 py-1 cursor-pointer hover:bg-[#A2A2A2] top-5 left-5" onClick={() => {setEditMenu_open(false); setEditingRecord(null);}}>
                            &lt; go back
                        </button>
                        <form className="w-full h-full flex flex-col justify-center items-center" onSubmit={handleUpdate}>
                            <table className="w-full max-w-md">
                                <tbody>
                                    <tr>
                                        <th className="py-4 text-left">date</th>
                                        <th className="py-4 text-center">
                                            <input 
                                                ...
                                                className="border-1 border-gray-400 rounded-md px-2 py-1 w-54"
                                            />
                                        </th>
                                    </tr>
                                    <tr>
                                        <th className="py-4 text-left">method</th>
                                        <th className="py-4 text-center">
                                            <select name="method" id="method" className="border-1 border-gray-400 rounded-md px-2 py-1 w-54" defaultValue={editingRecord.method}>
                                                <option value="">--選択してください--</option>
                                                <option value="convenience_store">コンビニ</option>
                                                <option value="food">飲食店</option>
                                                <option value="supermarket">スーパー</option>
                                    ...
                                    <tr>
                                        ...
                                        <th className="py-4 text-center">
                                            <input 
                                                type="text" 
                                                name="recorded_money" 
                                                defaultValue={editingRecord.recorded_money}
                                                placeholder="0" 
                                                className="border-1 border-gray-400 rounded-md px-2 py-1 w-54"
                                            />
                                        </th>
